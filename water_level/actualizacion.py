...
print('fecha inicial:%s'%now)


fechas = [now,now-datetime.timedelta(days=1),now,now-datetime.timedelta(days=2)]
fechas = []

for timedelta in np.arange(0,2):
    fechas.append(now-datetime.timedelta(days=timedelta))
        ...
        try:
            ...
            if series.dropna().index.size==0.0:
                ...
                try:
                    ...
                    scp = 'scp %s %s'%(filepath,remote_path+end.strftime('%Y%m%d')+'/')
                    print(os.system(scp))
                except:
                    print('no gif for : %s'%codigo)
                    
        except:
            print('no plot for : %s'%codigo)
